<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold text-gray-800">New Housekeeping Task</h2>
    </x-slot>

    <div class="py-6">
        <form method="POST" action="{{ route('housekeeping.store') }}" class="max-w-2xl mx-auto bg-white rounded-lg shadow p-6 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="task_type" required class="w-full rounded border-gray-300 text-sm">
                        ...
                        <option value="cleaning" @selected(old('task_type')=='cleaning')>Cleaning</option>
                        <option value="maintenance" @selected(old('task_type')=='maintenance')>Maintenance</option>
                        <option value="inspection" @selected(old('task_type')=='inspection')>Inspection</option>
                        <option value="turndown" @selected(old('task_type')=='turndown')>Turndown Service</option>
                        <option value="deep_clean" @selected(old('task_type')=='deep_clean')>Deep Clean</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="date" name="scheduled_date" value="{{ old('scheduled_date', date('Y-m-d')) }}" required class="w-full rounded border-gray-300 text-sm">
            ...
            <div class="flex gap-3 justify-end">
                ...
                <button type="submit" class="px-6 py-2.5 bg-emerald-600 text-white rounded-lg text-sm hover:bg-emerald-700 font-medium">Create Task</button>
            </div>
        </form>
    </div>
</x-app-layout>
